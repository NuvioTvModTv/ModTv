...
val officialClientProperties = Properties().apply {
    rootProject.file("config/nuvio-official-client.properties").inputStream().use { load(it) }
}
...
android {
    ...
    defaultConfig {
        ...
        minSdk = 24
        targetSdk = 36
        versionCode = env("NUVIO_CI_VERSION_CODE")?.let { raw ->
            requireNotNull(raw.toIntOrNull()?.takeIf { it > 1062 && it <= 2100000000 }) {
        ...
        buildConfigField("String", "GITHUB_OWNER", "\"NuvioTvModTv\"")
        buildConfigField("String", "GITHUB_REPO", "\"ModTv\"")
    }
    ...
    buildTypes {
        ...
        release {
            ...
            buildConfigField("String", "SUPABASE_URL", buildConfigString((localProperties.getProperty("NUVIO_SUPABASE_URL")?.trim()?.takeIf { it.isNotEmpty() } ?: officialClientProperties.getProperty("NUVIO_SUPABASE_URL"))))
            buildConfigField("String", "SUPABASE_ANON_KEY", buildConfigString((localProperties.getProperty("NUVIO_SUPABASE_ANON_KEY")?.trim()?.takeIf { it.isNotEmpty() } ?: officialClientProperties.getProperty("NUVIO_SUPABASE_ANON_KEY"))))
            buildConfigField("String", "SUPABASE_FALLBACK_URL", buildConfigString((localProperties.getProperty("NUVIO_SUPABASE_FALLBACK_URL")?.trim()?.takeIf { it.isNotEmpty() } ?: officialClientProperties.getProperty("NUVIO_SUPABASE_FALLBACK_URL"))))
            buildConfigField("String", "TV_LOGIN_WEB_BASE_URL", "\"${localProperties.getProperty("TV_LOGIN_WEB_BASE_URL", "https://nuvio.tv/tv-login")}\"")
            buildConfigField("String", "DEVICE_LOGIN_WEB_BASE_URL", "\"${localProperties.getProperty("DEVICE_LOGIN_WEB_BASE_URL", "https://nuvio.tv/link")}\"")
            buildConfigField("String", "PARENTAL_GUIDE_API_URL", "\"${localProperties.getProperty("PARENTAL_GUIDE_API_URL", "")}\"")
            buildConfigField("String", "INTRODB_API_URL", "\"${localProperties.getProperty("INTRODB_API_URL", "")}\"")
            ...
            buildConfigField("String", "SUPPORTERS_API_BASE_URL", buildConfigString(localProperties.getProperty("SUPPORTERS_API_BASE_URL", "https://nuvio.tv/")))
            buildConfigField("String", "SUPPORT_URL", buildConfigString(localProperties.getProperty("SUPPORT_URL", "https://nuvio.tv/support")))
            buildConfigField("String", "AVATAR_PUBLIC_BASE_URL", buildConfigString(localProperties.getProperty("AVATAR_PUBLIC_BASE_URL")?.trim()?.takeIf { it.isNotEmpty() } ?: officialClientProperties.getProperty("AVATAR_PUBLIC_BASE_URL")))
            buildConfigField("String", "UNIQUE_CONTRIBUTIONS_BASE_URL", "\"${localProperties.getProperty("UNIQUE_CONTRIBUTIONS_BASE_URL", "")}\"")
            buildConfigField("String", "PLAYBACK_REPORTS_BASE_URL", buildConfigString(localProperties.getProperty("PLAYBACK_REPORTS_BASE_URL", "")))
            buildConfigField("String", "PREMIUMIZE_CLIENT_ID", "\"${localProperties.getProperty("PREMIUMIZE_CLIENT_ID", "")}\"")
            buildConfigField("String", "SPONSOR_NAMES", buildConfigString(sponsorNames))
    ...
    splits {
        abi {
            ...
            reset()
            // Do not use IDE android.injected.build.abi: it can mark APKs test-only.
            val liveTvAbi = providers.gradleProperty("liveTvAbi").orNull
            if (liveTvAbi == null) {
                include("armeabi-v7a", "arm64-v8a")
            } else {
                require(liveTvAbi in listOf("armeabi-v7a", "arm64-v8a"))
                include(liveTvAbi)
            }
            isUniversalApk = true
        }
